...
def itk_image_to_multiscale(
    ...
    if (
        anatomical_axes
        and image_dimension == 3
        and not (np.eye(image_dimension) == input_direction).all()
    ):
        ...
        oriented_image = itk.orient_image_filter(
            ...
            desired_coordinate_orientation=desired_orientation,
        )

    elif anatomical_axes and image_dimension != 3:
        raise ValueError(
            f"Cannot use anatomical axes for input image of size {image_dimension}"
        )

    image_da = itk.xarray_from_image(oriented_image)
    image_da.name = name

    image_dims: Tuple[str, str, str, str] = (
        "x",
        "y",
        "z",
        "t",
    )  # ITK dims are in xyzt order
    ...
    spatial_image = to_spatial_image(
        image_da.data,
        dims=image_da.dims,
        scale=scale,
    ...
    return to_multiscale(spatial_image, scale_factors, method=method, chunks=chunks)
